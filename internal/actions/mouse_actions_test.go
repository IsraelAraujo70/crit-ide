...
func TestMouseClick_TextArea(t *testing.T) {
	...
	// Click at screenX=5, screenY=2 (row 0=tab bar, 1=border) → visualCol = 5-4 = 1 → byte offset 1.
...
func TestMouseClick_SecondLine(t *testing.T) {
	...
	// Click at screenY=3 (tab bar=0, border=1, first line=2) → buffer row 1.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 6, ScreenY: 3})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_WithScrollOffset(t *testing.T) {
	...
	// scrollY=5, screenY=4 (tab bar=0, border=1) → editorScreenY=2 → bufferRow = 5+2 = 7.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 4, ScreenY: 4})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_Gutter(t *testing.T) {
	...
	// Click on gutter (screenX=2 < gutterWidth=4), screenY=2 (after tab bar + border).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 2, ScreenY: 2})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_Statusline(t *testing.T) {
	...
	// Click on statusline (screenY=26 = headerRows + vpHeight).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 5, ScreenY: 26})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_BeyondEOF(t *testing.T) {
	...
	// Click at screenY=12 (header offset), but buffer only has 2 lines. Should clamp to last line.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 4, ScreenY: 12})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_BeyondLineEnd(t *testing.T) {
	...
	// Click at screenX=gutterWidth+50, screenY=2 (after tab bar + border), but line "hi" is only 2 chars.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 54, ScreenY: 2})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_TabLine(t *testing.T) {
	...
	// Click at screenX=4+3=7, screenY=2 (after tab bar + border) → visualCol=3, in the middle of tab expansion.
	// Should snap to byte offset 0 (the tab char).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 7, ScreenY: 2})
	reg.Execute("mouse.click", ctx)
	...
	// Click at screenX=4+4=8, screenY=2 → visualCol=4, which is after the tab → byte offset 1 ('h').
	ctx2 := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 8, ScreenY: 2})
