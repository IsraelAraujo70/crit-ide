...
func TestMouseClick_TextArea(t *testing.T) {
	...
	// Click at screenX=5, screenY=0 → visualCol = 5-4 = 1 → byte offset 1.
...
func TestMouseClick_SecondLine(t *testing.T) {
	...
	// Click at screenY=1 → buffer row 1.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 6, ScreenY: 1})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_WithScrollOffset(t *testing.T) {
	...
	// scrollY=5, screenY=2 → bufferRow = 7.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 4, ScreenY: 2})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_Gutter(t *testing.T) {
	...
	// Click on gutter (screenX=2 < gutterWidth=4).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 2, ScreenY: 0})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_Statusline(t *testing.T) {
	...
	// Click on statusline (screenY=24 = vpHeight).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 5, ScreenY: 24})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_BeyondEOF(t *testing.T) {
	...
	// Click at screenY=10, but buffer only has 2 lines. Should clamp to last line.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 4, ScreenY: 10})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_BeyondLineEnd(t *testing.T) {
	...
	// Click at screenX=gutterWidth+50, but line "hi" is only 2 chars.
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 54, ScreenY: 0})
	reg.Execute("mouse.click", ctx)
...
func TestMouseClick_TabLine(t *testing.T) {
	...
	// Click at screenX=4+3=7 → visualCol=3, which is in the middle of tab expansion.
	// Should snap to byte offset 0 (the tab char).
	ctx := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 7, ScreenY: 0})
	reg.Execute("mouse.click", ctx)
	...
	// Click at screenX=4+4=8 → visualCol=4, which is after the tab → byte offset 1 ('h').
	ctx2 := newTestContext(app, "mouse.click", events.MouseClickPayload{ScreenX: 8, ScreenY: 0})
