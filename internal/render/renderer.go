...
type ViewState struct {
	...
	// Command palette.
	Palette       *editor.PaletteState    // Non-nil when command palette is active.

	// Project search.
	ProjectSearch *editor.ProjectSearchState // Non-nil when project search is active.
...
func (r *Renderer) Render(vs *ViewState) {
	...
	if vs.Palette != nil {
		r.renderPalette(vs.Palette, vs.Width, vs.Height)
	}
...
// renderProjectSearch draws the project-wide search popup.
func (r *Renderer) renderProjectSearch(ps *editor.ProjectSearchState, screenW, screenH int) {
	// Popup dimensions.
	popupWidth := screenW * 2 / 3
	if popupWidth < 50 {
		popupWidth = 50
	...
	// Styles.
	borderStyle := tcell.StyleDefault.
		Foreground(tcell.NewRGBColor(80, 200, 120)).
		Background(tcell.NewRGBColor(20, 20, 30))
	headerStyle := tcell.StyleDefault.
		Foreground(tcell.NewRGBColor(120, 220, 160)).
		Background(tcell.NewRGBColor(20, 20, 30))
	inputBg := tcell.StyleDefault.
		Foreground(tcell.ColorWhite).
		Background(tcell.NewRGBColor(30, 30, 45))
	...
	selectedStyle := tcell.StyleDefault.
		Foreground(tcell.ColorWhite).
		Background(tcell.NewRGBColor(40, 80, 60)).
		Bold(true)
	fileHeaderStyle := tcell.StyleDefault.
		Foreground(tcell.NewRGBColor(80, 200, 120)).
		Background(tcell.NewRGBColor(20, 20, 30)).
		Bold(true)
	fileHeaderSelectedStyle := tcell.StyleDefault.
		Foreground(tcell.NewRGBColor(120, 255, 160)).
		Background(tcell.NewRGBColor(40, 80, 60)).
		Bold(true)
	emptyStyle := tcell.StyleDefault.
		Foreground(tcell.NewRGBColor(100, 100, 100)).
	...
	// Show cursor inside project search input.
	r.screen.ShowCursor(cursorScreenX, inputRow)

	// Draw result count on the right of input row.
	countStr := fmt.Sprintf("%d/%d files ", ps.TotalHits, ps.TotalFiles)
	if ps.TotalHits == 0 && !ps.HasResults() {
		countStr = "Enter to search "
	}
	countX := px + popupWidth - 1 - len(countStr)
	if countX > cx+1 {
		for _, ch := range countStr {
	...
	// Draw result rows.
	resultStartRow := sepRow + 1
	for row := 0; row < maxVisible; row++ {
		screenRow := resultStartRow + row
		r.screen.SetContent(px, screenRow, tcell.RuneVLine, nil, borderStyle)

		entryIdx := ps.ScrollY + row
		if entryIdx < len(ps.Entries) {
			entry := ps.Entries[entryIdx]
			isSelected := entryIdx == ps.SelectedIdx

			var style tcell.Style
			if entry.IsHeader {
				if isSelected {
					style = fileHeaderSelectedStyle
				} else {
			...
			for x := 1; x < popupWidth-1; x++ {
				r.screen.SetContent(px+x, screenRow, ' ', nil, style)
			}
			...
			// Empty row.
			for x := 1; x < popupWidth-1; x++ {
				r.screen.SetContent(px+x, screenRow, ' ', nil, emptyStyle)
			}
		}
	...
	// Draw bottom border.
	bottomRow := resultStartRow + maxVisible
	r.screen.SetContent(px, bottomRow, tcell.RuneLLCorner, nil, borderStyle)
	for x := 1; x < popupWidth-1; x++ {
		r.screen.SetContent(px+x, bottomRow, tcell.RuneHLine, nil, borderStyle)
	}
